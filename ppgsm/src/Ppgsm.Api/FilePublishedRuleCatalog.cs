...
public sealed class FilePublishedRuleCatalog(string catalogPath, string profilePath, string trustedVersion, string trustedAttestation) : IPublishedRuleCatalog
{
    ...
    private static bool IsValidRule(RuleDefinition rule) =>
        ...
        !string.IsNullOrWhiteSpace(rule.Evaluator.Key) && rule.Evaluator.Params.ValueKind == JsonValueKind.Object &&
        rule.EvidenceRequirements.Count > 0 && rule.EvidenceRequirements.All(evidence =>
            !string.IsNullOrWhiteSpace(evidence.Section) && evidence.Coverage is "Full" &&
            !string.IsNullOrWhiteSpace(evidence.Confidence) && evidence.Paths.Count > 0) &&
